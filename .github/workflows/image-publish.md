# GHA Reusable Workflow Overview: `image-publish.yml`
...
This GitHub Actions **reusable workflow** builds and pushes Docker images to Docker Hub, GitHub Container Registry (GHCR), Harbor, or other container registries, and optionally requests Singularity image builds or removals on CVMFS.
...
| `tag_suffix`      | no (`""`)          | Optional tag suffix appended to all generated tags.<br>Examples: `foo` → `v1.2.3-foo`, `main-foo`, `latest-foo`. If provided, `latest` automatically becomes `latest-foo`.                                                                                            |
...
*All CVMFS Singularity image builds are handled by [`WIPACrepo/cvmfs-actions`](https://github.com/WIPACrepo/cvmfs-actions) and listed in its [docker_images.txt](https://github.com/WIPACrepo/cvmfs-actions/blob/main/docker_images.txt).*
...
* All tags generated by [`docker/metadata-action`](https://github.com/docker/metadata-action) will include the suffix, e.g.,
  `v1.2.3-appt`, `v1.2-appt`, `v1-appt`, `main-appt`, and `latest-appt`.
* The plain `latest` tag will **not** be generated.

This makes it easy to publish and track per-flavor builds (e.g., `-appt`, `-gpu`, `-dev`) without changing the base versioning scheme.

## Example Usage

Here are a few examples, out of many possible configurations.

### Single-Action / Simple Workflows
...
### Multi-Action / Dynamic Workflows

Build and/or remove for GHCR + CVMFS:
...
jobs:
  image-publish:
    ...
    with:
      ...
      image_name: myrepo
      mode: BUILD
      tag_suffix: ${{ matrix.flavor }}
      build_args: |
        THE_BUILD_FLAVOR=${{ matrix.flavor }}
```
